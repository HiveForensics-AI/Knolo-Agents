...
in `.knolo/agents/` (project-level) or `~/.knolo/agents/` (user-level).
...
let def = AgentDefinition::from_file(".knolo/agents/code-reviewer.md")?;
...
// Find all .md files in .knolo/agents/ directories
...
1. **Project-level** (highest priority): `.knolo/agents/*.md` — walk
   from `cwd` up to the git repository root. Files found closer to
   `cwd` take priority.
2. **User-level**: `~/.knolo/agents/*.md`
...
example, a project `.knolo/agents/code-reviewer.md` shadows a
