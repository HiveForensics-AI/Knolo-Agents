...
Hooks let a Knolo host run custom scripts or HTTP requests at key moments during
an agent session. This document describes extracted harness behavior; the
public CLI currently exposes only the parent workspace task/approval contract.
...
1. Create the hooks directory:
   ...
   2. Create a simple hook file, e.g. `~/.knolo/hooks/session-start.json`:
   ```json
   {
     "hooks": {
       "SessionStart": [
