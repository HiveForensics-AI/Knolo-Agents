> **Knolo product component**
> This component is distributed as part of the Knolo agent product. Public installation and user workflows are documented by the parent Knolo workspace. Historical upstream identifiers may remain in internal code for provenance; required licenses and notices remain authoritative.

# Hook Examples

Sample hooks for Knolo. Copy to `~/.knolo/hooks/` to enable globally, or to `<project>/.knolo/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.knolo/hooks/bin
cp examples/hooks/safe-shell.json ~/.knolo/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.knolo/hooks/bin/
chmod +x ~/.knolo/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.knolo/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.knolo/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.knolo/hooks/bin/
chmod +x ~/.knolo/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.knolo/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.knolo/hooks/bin
cp examples/hooks/session-log.json ~/.knolo/hooks/
cp examples/hooks/bin/session-log.sh ~/.knolo/hooks/bin/
chmod +x ~/.knolo/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.knolo/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.knolo/hooks/bin
cp examples/hooks/tool-logger.json ~/.knolo/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.knolo/hooks/bin/
chmod +x ~/.knolo/hooks/bin/tool-logger.sh
```
...
```sh
mkdir -p ~/.knolo/hooks/bin
cp examples/hooks/stop-verify.json ~/.knolo/hooks/
cp examples/hooks/bin/stop-verify.sh ~/.knolo/hooks/bin/
chmod +x ~/.knolo/hooks/bin/stop-verify.sh
```
...
- **Matcher:** regex on tool name. Claude names like `Bash`, `Read`, `Edit` are auto-expanded to also match Knolo names (`run_terminal_cmd`, `read_file`, `search_replace`)
...
Remove the JSON file from `~/.knolo/hooks/`. The hook stops running on the next session.