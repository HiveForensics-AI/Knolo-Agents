...
impl XaiProtoBuilder {
    ...
    fn emit_rerun_if_changed<'a>(
        ...
        for proto in protos {
            ...
            let prefix = "/dev/null:";
            // Newer protoc versions emit a `/dev/null:` makefile target for
            // `--dependency_out=/dev/stdout`. Older, still widely installed
            // versions emit the input proto path as the target instead. Both
            // forms carry the same dependency information for our rerun
            // tracking, so accept either when the legacy target matches the
            // proto being compiled.
            let rem = if let Some(rem) = first_line.strip_prefix(prefix) {
                rem
            } else {
                let legacy_target = first_line.trim().replace("//", "/");
                let proto_target = proto.to_string_lossy().replace("//", "/");
                if legacy_target == proto_target {
                    first_line.trim()
                } else {
                    return Err(anyhow::anyhow!(
                        "protoc command output must start with /dev/null: or name the input proto: {output:?}"
                    ));
                }
            };
