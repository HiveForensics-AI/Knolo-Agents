import { DefinitionError, validateDefinition, validateState } from "../../builder/index.js";
...
export class TypeScriptEngine<S> implements Engine<S> {
  ...
  execute(initial: S, options: RunOptions = {}): AsyncGenerator<ExecutionEventV1, ExecutionReport<S>> { validateState(this.definition.schema, initial); const snapshot: StateSnapshot<S> = { schema_id: this.definition.schema.id, revision: 0, value: structuredClone(initial), provenance: null }; return this.runFrom(snapshot, this.definition.graph.entry, 0, 0, 0, options.executionId ?? randomId(), options); }
