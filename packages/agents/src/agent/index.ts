...
import { assertReplayMatch, isReplayTrace, validateReplayEvents, type ReplayTraceV1 } from "../replay/index.js";
...
export class Agent<S, ResumeInput = JsonValue, Result = JsonValue> {
  ...
  replay(events: readonly ExecutionEventV1[]): readonly ExecutionEventV1[] { validateReplayEvents(events); return events.map(event => structuredClone(event)); }
  /**
   * Re-execute a portable graph and compare its control-plane trace (excluding
   * wall-clock timestamps). When `expected` includes snapshots, also fail closed
   * if per-revision state diverges even when event kinds still match.
   */
  async replayDeterministic(initial: S, expected: readonly ExecutionEventV1[] | ReplayTraceV1<S>): Promise<ExecutionReport<S, Result>> {
    const trace: ReplayTraceV1<S> = isReplayTrace(expected) ? expected : { version: 1, events: expected };
    validateReplayEvents(trace.events);
    validateState(this.definition.schema, initial);
    const report = await this.run(initial, { executionId: trace.events[0]?.execution_id });
    assertReplayMatch(report as ExecutionReport<unknown, unknown>, trace as ReplayTraceV1<unknown>);
    return report;
  }
